...
def generate_daily_report(caution_df, alarm_df, report_date):
    ...
    ax.set_yticks([])
    ax.set_title(f'{report_date} al {report_date_next_day} - Sensores Challcobamba', fontsize=16, pad=20, loc='left')
...
def generate_report(df, file_name):
    ...
    st.download_button(
        ...
        data=doc_buffer,
        file_name=f"REPORTE DIARIO CHALLCOBAMBA {report_date}-{report_date_next_day}.docx",
