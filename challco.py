...
def generate_daily_report(caution_df, alarm_df, report_date):
    ...
    return ax
#codigo numero 2 daily reports
def generate_daily_report_for_df2(df2_caution, df2_alarm, report_date):
    # Verificar si los DataFrames están vacíos antes de continuar
    if df2_caution.empty and df2_alarm.empty:
        print("Ambos DataFrames están vacíos. No se generará el reporte.")
        return None  # Return None if both DataFrames are empty

    # Si ambos DataFrames no están vacíos, eliminar la primera y última fila solo del primero
    if not df2_caution.empty and not df2_alarm.empty:
        df2_caution = df2_caution.iloc[1:-1]

    # Convert 'Date' to datetime for easier manipulation solo si no están vacíos
    if not df2_caution.empty:
        df2_caution['Date'] = pd.to_datetime(df2_caution['Date'])
    if not df2_alarm.empty:
        df2_alarm['Date'] = pd.to_datetime(df2_alarm['Date'])

    # Concatenar los DataFrames
    combined_df = pd.concat([df2_caution, df2_alarm], ignore_index=True)
...
def plot_eventos(df, report_date):
    ...
    return ax
#funcion plot eventos df2
def plot_eventos(df2, report_date):  # Cambiado 'df' por 'df2'
    # Conversión de fechas
    df2['Start'] = pd.to_datetime(df2['Start'])
    df2['hora'] = df2['Start'].dt.hour

    # Contar los eventos por hora y por tipo
    contador_tipo_1 = df2[df2['Description'] == 'Caution Flash'].groupby('hora').size()  # Amarillo
    contador_tipo_2 = df2[df2['Description'] == 'Alarm Flash'].groupby('hora').size()  # Roja
    contador_tipo_3 = df2[df2['Description'] == 'Warning Flash'].groupby('hora').size()  # Naranja
    ...
    # Devolver el gráfico sin mostrarlo
    return ax
#definición generate report para df 
...
#generate report para df 2
   def generate_report(df2, file_name):
    report_date = get_reporte_date(file_name)

    # Verificar las descripciones únicas en la columna 'Description'
    unique_descriptions = df2['Description'].dropna().unique()  # Ignorar valores nulos
    ...
    for description in unique_descriptions:
        # Filtrar el DataFrame original para cada descripción
        filtered_df2 = df2[df2['Description'] == description]

        # Crear un nuevo DataFrame con pares Start-End si 'End' no tiene valores nulos
        if filtered_df2['End'].isna().any():
            # Si hay valores nulos en 'End', agregar el DataFrame original sin reorganizar
            description_dfs2[description] = filtered_df2
        else:
            # Reorganizar el DataFrame si no hay valores nulos en 'End'
            reorganized_data = []
            ...
            # Convertir los datos reorganizados a un DataFrame
            reorganized_df2 = pd.DataFrame(reorganized_data)

            # Obtener la fecha del primer registro y establecer la hora 00:00
            first_date = pd.to_datetime(reorganized_df2['Date'].iloc[0]).normalize()
            start_of_day = first_date.strftime('%m/%d/%y %H:%M %p')

            # Crear un registro al inicio
            reorganized_df2 = pd.concat([
                pd.DataFrame([{
                    'Date': start_of_day,
                    'Description': description,
                    'Type': '-',
            ...
            # Obtener la fecha del último registro y sumar un día para establecer la hora 00:00
            last_date = pd.to_datetime(reorganized_df2['Date'].iloc[-1]).normalize() + timedelta(days=1)
            start_of_next_day = last_date.strftime('%m/%d/%y %H:%M %p')
            ...
            # Convertir la columna 'Date' a datetime para asegurarse que se pueda calcular la duración
            reorganized_df2['Date'] = pd.to_datetime(reorganized_df2['Date'])

            # Crear columna de Duration en formato min:segundos
            durations = []
            for i in range(len(reorganized_df2) - 1):
                end_time = reorganized_df2.iloc[i + 1]['Date']
                duration = end_time - reorganized_df2.iloc[i]['Date']
                durations.append(duration)
            ...
            # Asignar la columna de duración al DataFrame reorganizado
            reorganized_df2['Duration'] = durations

            # Agregar al diccionario
            description_dfs2[description] = reorganized_df2

    # Generar la gráfica Diaria
    caution_df2 = description_dfs2.get('Caution', pd.DataFrame())  # Si no existe, devuelve un DataFrame vacío
    alarm_df2 = description_dfs2.get('Alarm', pd.DataFrame())  # Si no existe, devuelve un DataFrame vacío
    ax_1 = generate_daily_report(caution_df2, alarm_df2, report_date)  # Get ax object from the report function
...
# Función para cargar y mostrar archivo para df
def cargar_archivo():
