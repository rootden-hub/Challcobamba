...
#funcion de barrido
def generate_daily_report(caution_df2, alarm_df2, report_date):
    # Aquí, `caution_df2` y `alarm_df2` son DataFrames ya filtrados y reorganizados
    # Lógica para generar la gráfica diaria
    pass
...
def process_and_generate_report(df2, file_name):
    ...
    for description in unique_descriptions:
        ...
        # Si 'End' tiene valores nulos, no reorganizamos
        if filtered_df2['End'].isna().any():
            description_dfs2[description] = filtered_df2
        else:
            # Reorganizar el DataFrame si no hay valores nulos en 'End'
            reorganized_df2 = reorganize_data(filtered_df2, description)
    ...
    ax_1 = generate_daily_report(caution_df2, alarm_df2, report_date)  # Gráfica diaria
    ax_2 = plot_eventos(df2, report_date)  # Gráfico de eventos por hora
    ...
    # Guardar las gráficas como imágenes en buffers de memoria
    img_buf_1 = BytesIO()
    ax_1.figure.savefig(img_buf_1, format='png')
    img_buf_1.seek(0)
    ...
    # Agregar logo y título en el header
    header = doc.sections[0].header
    header_table = header.add_table(rows=1, cols=2, width=doc.sections[0].page_width)
    header_table.columns[0].width = Pt(50)
    ...
    # Insertar imágenes de las gráficas en el documento
    doc.add_picture(img_buf_1, width=Pt(650))
    last_paragraph = doc.paragraphs[-1]
    last_paragraph.alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
    ...
    footer_paragraph.text = f"{today_date} Supervisión de Mantenimiento en Telecomunicaciones"
    ...
    # Guardar y generar el archivo descargable
    doc_buffer = BytesIO()
    doc.save(doc_buffer)
    doc_buffer.seek(0)
