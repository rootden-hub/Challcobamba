...
def generate_daily_report(caution_df, alarm_df, report_date):
    ...
    combined_df['Date'] = pd.to_datetime(combined_df['Date'])  # Asegurarse de que sea datetime
    combined_df['Date'] = combined_df['Date'] - pd.Timedelta(hours=7)  # Retroceder 7 horas
    combined_df['Date'] = combined_df['Date'].dt.floor('D') + pd.Timedelta(hours=7)  # Fijar a las 7:00 AM
